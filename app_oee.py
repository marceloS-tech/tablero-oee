...
# =========================================================================
# RECEPTOR DE GOLPES SEGURO CON ARCHIVO DE RESPALDO EN DISCO
# =========================================================================
ARCHIVO_GOLPES = "contador_golpes.txt"

if not os.path.exists(ARCHIVO_GOLPES):
    with open(ARCHIVO_GOLPES, "w") as f:
        f.write("0")

with open(ARCHIVO_GOLPES, "r") as f:
    golpes_totales = int(f.read().strip())
...
if "evento" in query_params and query_params["evento"] == "golpe" and "maquina" in query_params:
    if query_params["maquina"] == "M1":
        golpes_totales += 1
        with open(ARCHIVO_GOLPES, "w") as f:
            f.write(str(golpes_totales))
        st.success("Impacto IoT registrado en disco")
...
if filas_maquina.any():
    ...
    st.session_state.db_historial_planta.at[ultimo_idx, "Buenas"] += golpes_totales
    
    if golpes_totales > 0:
        st.session_state.db_objetivos[maquina_nombre]["Estado_TR"] = "PRODUCIENDO"
        st.session_state.db_objetivos[maquina_nombre]["Ultimo_Evento"] = f"Contando Golpes ({golpes_totales} registrados)"

# =========================================================================
# 3. CONTROL DE SESIÓN Y PARED DE LOGUEO (PAYWALL)
# =========================================================================
...
# Función auxiliar para los gauges
...
if "1." in capa_activa:
    ...
    with col_op1:
        operario_nom = st.selectbox("Seleccione su Nombre y Apellido:", ["VILLARROEL ENZO", "FRANCO MAXIMILIANO", "MOREIRA CRISTIAN"])
    ...
    with col_po2:
        falla_op = st.selectbox("Causa Raíz de la detención:", ["Ninguna", "Falla en la mesa de trabajo", "Falla Robótica", "Asistencia de Mantenimiento", "Ajuste de Banco", "Soporte del Líder"])
    ...
    if st.button("💾 Enviar Bloque Hora a Hora a la Nube", use_container_width=True):
        nuevo_reg = {
            ...
            "Operario": operario_nom, "Meta": meta_automatica, "Buenas": buenas_op, "Retrabajo": ret_op,
            "Observadas": obs_op, "Min_Parada": minutos_op, "Falla": falla_op, "Es_Falla_Tecnica": es_tec, "Validado": False
        }
...
# =========================================================================
# CONTROL AUTOMÁTICO DE PANTALLA Y REFRESCO DE DATOS (MÓDULO TV)
# =========================================================================
st.markdown("""
<script>
